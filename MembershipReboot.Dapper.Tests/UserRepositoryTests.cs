...
namespace MembershipReboot.Dapper.Tests {
    public class UserRepositoryTests : IDisposable, IClassFixture<CheckDatabase> {
        #region Fields
        ...
        private static DapperUserAccountRepository<RelationalUserAccount> CreateRepository(IDbConnection connection) {
            var repo = new DapperUserAccountRepository<RelationalUserAccount>(connection);
            return repo;
        }

        private static DapperUserAccountRepository<RelationalUserAccount> CreateRepository(IDbConnection connection, string userAccountTable,
            ...
            var repo = new DapperUserAccountRepository<RelationalUserAccount>(connection, userAccountTable: userAccountTable,
        ...
        private Guid GetFirstID(SqlConnection conn) {
            var id = conn.Query<Guid>("select ID from UserAccounts").FirstOrDefault();
            Assert.True(id != Guid.Empty);
            return id;
        }

        private int GetFirstKey(SqlConnection conn) {
            var key = conn.Query<int>("select [Key] from UserAccounts").FirstOrDefault();
            Assert.True(key > 0);
            return key;
        }

        private void AssertUserCount(SqlConnection conn, int count) {
            Assert.Equal(count, conn.Query<int>("select count(*) from UserAccounts").FirstOrDefault());
        }
        ...
        private void AssertUserIsValid(RelationalUserAccount user, bool checkKey = true) {
            Assert.NotNull(user);
            if (checkKey) {
        ...
        private void AssertUsersEqual(RelationalUserAccount user1, RelationalUserAccount user2, bool compareKey = true, bool compareCollections = false) {
        ...
        private RelationalUserAccount CreateUser(DapperUserAccountRepository<RelationalUserAccount> repo, bool includeChildren = true) {
        ...
        public void DefaultTableNamesAreSet() {
            using (var conn = CreateConnection()) {
                var repo = CreateRepository(conn);
                Assert.Equal("UserAccounts", repo.UserAccountTable);
        ...
        public void CanAddNewUser() {
            using (var conn = CreateConnection()) {
                ...
                var userFromDb = conn.Query<RelationalUserAccount>("select * from UserAccounts").FirstOrDefault();
