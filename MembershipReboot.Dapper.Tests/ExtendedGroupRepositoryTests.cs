...
namespace MembershipReboot.Dapper.Tests {
    public class ExtendedGroupRepositoryTests : IDisposable, IClassFixture<CheckDatabase> {
        #region Fields
        ...
        public ExtendedGroupRepositoryTests() { }
        ...
        private static DapperGroupRepository<ExtendedGroup> CreateRepository(IDbConnection connection, string groupName, string childGroupName) {
            var repo = new DapperGroupRepository<ExtendedGroup>(connection, null, "dbo", groupName, new Dictionary<Type, string> {
        ...
        public void DefaultTableNamesAreSet() {
            using (var conn = CreateConnection()) {
                var repo = CreateRepository(conn);
                Assert.Equal("ExtendedGroups", repo.GroupTable);
        ...
        public void AlternativeTableNamesAreSet() {
            using (var conn = CreateConnection()) {
                var repo = CreateRepository(conn, "group_table", "group_child_table");
                Assert.Equal("group_table", repo.GroupTable);
                Assert.Equal("group_child_table", repo.TableNameMap[typeof(RelationalGroupChild)]);
            }
        }
        ...
        public void AlternativeTableNamesAreEscaped() {
            using (var conn = CreateConnection()) {
                var repo = CreateRepository(conn, "gro[up_ta]ble", "gr][oup_child_t]]able");
                Assert.Equal("gro[[up_ta]]ble", repo.GroupTable);
                Assert.Equal("gr]][[oup_child_t]]]]able", repo.TableNameMap[typeof(RelationalGroupChild)]);
            }
        }
        ...
        [Fact]
        public void CreateReturnsNonNullExtendedGroup() {
            using (var conn = CreateConnection()) {
                var repo = CreateRepository(conn);

                var newGroup = repo.Create();
                Assert.NotNull(newGroup);
                Assert.IsAssignableFrom<ExtendedGroup>(newGroup);
        ...
        public void CanAddNewGroup() {
            using (var conn = CreateConnection()) {
                ...
                var group = repo.Create()
                     ...
                     .SetField("details", m => m.Details)
                     .SetField(DateTime.Now, m => m.Created)
                     .SetField(DateTime.Now, m => m.LastUpdated);

                repo.Add(group);

                var count = conn.Query<int>("select count(*) from ExtendedGroups").FirstOrDefault();
                Assert.Equal(1, count);

                var groupFromDb = conn.Query<ExtendedGroup>("select * from ExtendedGroups").FirstOrDefault();
                ...
                Assert.Equal(group.Details, groupFromDb.Details);
        ...
        public void CanAddNewGroupWithChildren() {
            using (var conn = CreateConnection()) {
                ...
                var groupCount = conn.Query<int>("select count(*) from ExtendedGroups").FirstOrDefault();
        ...
        public void ChildrenOfGroupHaveCorrectParentID() {
            using (var conn = CreateConnection()) {
                ...
                var groupKey = conn.Query<int>("select [Key] from ExtendedGroups").FirstOrDefault();
        ...
        public void CanGetGroupByID() {
            using (var conn = CreateConnection()) {
                ...
                conn.Execute("insert into ExtendedGroups values (newid(), 'default', 'name', 'details', getdate(), getdate())");
                var groupCount = conn.Query<int>("select count(*) from ExtendedGroups").FirstOrDefault();
                Assert.Equal(1, groupCount);
                var id = conn.Query<Guid>("select ID from ExtendedGroups").FirstOrDefault();
                ...
                Assert.Equal("details", group.Details);
            }
        }
        ...
        public void CanGetGroupByIDWithChildren() {
            using (var conn = CreateConnection()) {
                ...
                var groupCount = conn.Query<int>("select count(*) from ExtendedGroups").FirstOrDefault();
                Assert.Equal(1, groupCount);
                var key = conn.Query<int>("select [Key] from ExtendedGroups").FirstOrDefault();
                Assert.True(key > 0);
                var id = conn.Query<Guid>("select ID from ExtendedGroups").FirstOrDefault();
                Assert.True(id != Guid.Empty);
        ...
        public void CanGetMultipleGroupsByMultipleIDs() {
            using (var conn = CreateConnection()) {
                ...
                conn.Execute("insert into ExtendedGroups values (newid(), 'default', 'name', 'details', getdate(), getdate()), (newid(), 'default', 'name2', 'details', getdate(), getdate())");
                var groupCount = conn.Query<int>("select count(*) from ExtendedGroups").FirstOrDefault();
                Assert.Equal(2, groupCount);
                var ids = conn.Query<Guid>("select ID from ExtendedGroups");
                Assert.NotNull(ids);
        ...
        public void CanGetMultipleGroupsByMultipleIDsWithChildren() {
            using (var conn = CreateConnection()) {
                ...
                var groupCount = conn.Query<int>("select count(*) from ExtendedGroups").FirstOrDefault();
                Assert.Equal(2, groupCount);
                var keys = conn.Query<int>("select [Key] from ExtendedGroups");
                Assert.NotNull(keys);
                var ids = conn.Query<Guid>("select ID from ExtendedGroups");
                Assert.NotNull(ids);
        ...
        public void CanUpdateGroup() {
            using (var conn = CreateConnection()) {
                ...
                groupFromDb.SetField("new details", m => m.Details);
                ...
                Assert.Equal("new details", groupFromDb.Details);
        ...
        public class ExtendedGroup : RelationalGroup {
